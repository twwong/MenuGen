...
**Current status:** Milestone 1 is complete. Milestone 2 is in progress. The deterministic creator proof runs from ordered upload through review, one-time anonymous claim, quota reservation/consumption, source-photo reuse, independent item generation, visible partial failure, uploader regeneration, a resumable dashboard, a private result, one-time completion, and idempotent deletion/expiry. The managed path now implements the same journey across Clerk, Neon, private Blob stores, Transloadit, Vercel Workflow, Upstash, Resend, and OpenAI. Clerk, the Singapore Neon development database, separate private Singapore Blob stores, the Transloadit development workspace, the Singapore Upstash Redis rate-limit database, Resend's development sender, and the authenticated Vercel Workflow preview are provisioned; all committed migrations are applied. The isolated OpenAI preview project exists, but API billing is not enabled, so the project key and enforced spend limit remain blocked. Protected-preview integration tests and manually approved live smoke tests also remain open.
...
- [ ] Authentication and generation: local code now covers Clerk-backed atomic claiming, a transactional Neon quota ledger, source-only completion, grouped Vercel Workflow fan-out, bounded retries, item isolation, cost ceilings, private result assets, and two uploader regenerations. The authenticated Preview deployment compiles all seven workflows and nineteen steps, but the creator flag remains off. An isolated personal OpenAI project exists, but it has no API billing or credits; no key, spend limit, saved model policy, Vercel secret, or provider call was created. Concurrency and provider-failure behavior still need provisioned integration tests after OpenAI configuration resumes.
- [ ] Resume and lifecycle: local code now covers safe polling DTOs, private asset streaming, the dashboard, database-backed Resend idempotency, immediate deletion, 24-hour source cleanup, 30-day expiration, and 90-day audit pruning. Resend's sending-only development key and test sender are configured for Development and Preview. A content-free test request was rejected before send with sanitized reason `validation_error`, so scheduled execution, end-to-end email delivery, and remote asset deletion still need protected-preview verification.
