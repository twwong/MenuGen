...
- [ ] Authentication and generation: local code now covers Clerk-backed atomic claiming, a transactional Neon quota ledger, source-only completion, grouped Vercel Workflow fan-out, bounded retries, item isolation, cost ceilings, private result assets, and two uploader regenerations. The authenticated Preview deployment compiles all seven workflows and nineteen steps, but the creator flag remains off. Concurrency and provider-failure behavior still need provisioned integration tests after OpenAI configuration.
